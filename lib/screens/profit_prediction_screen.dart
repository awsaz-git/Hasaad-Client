...
class _ProfitPredictionScreenState extends State<ProfitPredictionScreen> {
  ...
  final Color modelThemeColor = const Color(0xFF00C897); // Profit theme color
  ...
  Future<void> _submitPrediction() async {
    ...
    if (_saleDate!.isBefore(_plantingDate!) || _saleDate!.isAtSameMomentAs(_plantingDate!)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.translate('sale_date_after_planting'))),
      );
      return;
    }
    ...
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('$backendUrl/predict_profit?t=$timestamp'));
      ...
      debugPrint('Profit Prediction error: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showResultOverlay(Map<String, dynamic> result) {
    ...
    double numericValue = double.tryParse(rawValue.replaceAll(RegExp(r'[^0-9.-]'), '')) ?? 0.0;
    bool isLoss = numericValue < 0;
    
    String formattedAmount = "${numericValue.abs().toStringAsFixed(2)} ${l10n.translate('jod')}";
    final selectedCrop = _allCrops.firstWhere((c) => c.id == _selectedCropId);
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              isLoss ? l10n.translate('estimated_loss') : l10n.translate('estimated_profit'), 
              style: GoogleFonts.cairo(fontSize: 18, color: isLoss ? Colors.red[700] : darkGreen, fontWeight: FontWeight.bold)
            ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: (isLoss ? Colors.red : modelThemeColor).withOpacity(0.1),
                borderRadius: BorderRadius.circular(24),
                border: Border.all(color: (isLoss ? Colors.red : modelThemeColor).withOpacity(0.2)),
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(isLoss ? Icons.trending_down : Icons.trending_up, color: isLoss ? Colors.red : modelThemeColor, size: 36),
                  const SizedBox(width: 16),
                  Text(formattedAmount, style: GoogleFonts.cairo(fontSize: 36, fontWeight: FontWeight.bold, color: isLoss ? Colors.red : modelThemeColor)),
                ],
              ),
            ),
            ...
            _buildResultRow(
              ...
              '', 
              darkGreen,
              trailing: _buildCropBadge(selectedCrop, lang, darkGreen),
            ),
            const SizedBox(height: 16),
            _buildResultRow(Icons.square_foot_outlined, l10n.translate('area_donums'), "${result['area_donums']} ${l10n.translate('dunums')}", darkGreen),
            const SizedBox(height: 16),
            _buildResultRow(Icons.location_on_outlined, l10n.translate('governorate'), _userGov?.getName(lang) ?? '', darkGreen),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: darkGreen, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)), elevation: 0),
  ...
  Widget _buildCropBadge(Crop crop, String lang, Color color) => Container(
    ...
    child: Row(
      ...
      children: [
        ...
        Text(crop.getName(lang), style: GoogleFonts.cairo(fontWeight: FontWeight.bold, color: color, fontSize: 14)),
      ],
    ),
  );

  Widget _buildResultRow(IconData icon, String label, String value, Color color, {Widget? trailing}) {
    return Row(
      children: [
        Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: color.withOpacity(0.05), shape: BoxShape.circle), child: Icon(icon, color: color, size: 20)),
        const SizedBox(width: 16),
        Text(label, style: GoogleFonts.cairo(color: Colors.grey[600], fontSize: 14)),
        const Spacer(),
        trailing ?? Text(value, style: GoogleFonts.cairo(fontWeight: FontWeight.bold, color: Colors.black87, fontSize: 16)),
      ],
    );
  }
          ...
          _isLoading 
            ...
            : SingleChildScrollView(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _buildInfoCard(l10n.translate('profit_prediction'), l10n.translate('profit_model_detail'), modelThemeColor, darkGreen),
                    const SizedBox(height: 24),
                    ...
                    Form(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 40),
                          _buildSubmitButton(l10n.translate('predict_profit'), modelThemeColor),
                          const SizedBox(height: 40),
                        ],
                      ),
                    ),
  ...
  Widget _buildInfoCard(String title, String detail, Color primary, Color dark) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: primary.withOpacity(0.1)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 10, offset: const Offset(0, 4))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Icon(Icons.info_outline, color: primary, size: 20), const SizedBox(width: 8), Text(title, style: GoogleFonts.cairo(fontWeight: FontWeight.bold, color: dark))]),
        const SizedBox(height: 8),
        Text(detail, style: GoogleFonts.cairo(fontSize: 14, color: Colors.grey[700], height: 1.5)),
      ]),
    );
  }
