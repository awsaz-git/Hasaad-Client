...
class _PricePredictionScreenState extends State<PricePredictionScreen> {
  ...
  final Color modelThemeColor = const Color(0xFFFF9F43); // Price theme color (Orange)
  ...
  Future<void> _submitPrediction() async {
    ...
    if (!_formKey.currentState!.validate() || _selectedCropId == null || _selectedMarketGovId == null || _saleDate == null) {
    ...
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('$backendUrl/predict_market_price'));
  ...
  void _showResultOverlay(Map<String, dynamic> result) {
    ...
    String formattedPrice = "${numericValue.toStringAsFixed(2)} ${l10n.translate('jod_per_ton')}";
    final selectedCrop = _allCrops.firstWhere((c) => c.id == _selectedCropId);
    final marketGov = _governorates.firstWhere((g) => g.id == _selectedMarketGovId);
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(l10n.translate('price_result'), style: GoogleFonts.cairo(fontSize: 18, color: darkGreen, fontWeight: FontWeight.bold)),
              ...
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.sell, color: modelThemeColor, size: 36),
                  const SizedBox(width: 16),
                  Flexible(
                    child: Text(formattedPrice, style: GoogleFonts.cairo(fontSize: 28, fontWeight: FontWeight.bold, color: modelThemeColor), overflow: TextOverflow.ellipsis),
                  ),
                ]),
              ),
              ...
              const SizedBox(height: 16),
              _buildResultRow(Icons.calendar_today_outlined, l10n.translate('sale_date'), DateFormat('yyyy-MM-dd').format(_saleDate!), darkGreen),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(l10n.translate('price_prediction'), style: GoogleFonts.cairo(color: darkGreen, fontWeight: FontWeight.bold)), backgroundColor: Colors.white, elevation: 0, centerTitle: true, iconTheme: const IconThemeData(color: darkGreen)),
      body: Stack(
        children: [
          _isLoading 
            ...
            : SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _buildInfoCard(l10n.translate('price_prediction'), l10n.translate('price_model_detail'), modelThemeColor, darkGreen),
                  ...
                  Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    _buildLabel(l10n.translate('sale_date'), darkGreen),
                    const SizedBox(height: 8),
                    _buildDatePicker(_saleDate, () => _selectDate(context), modelThemeColor, darkGreen, l10n),

                    const SizedBox(height: 40),
                    _buildSubmitButton(l10n.translate('predict_price'), modelThemeColor),
                    const SizedBox(height: 40),
                  ])),
                ]),
              ),
