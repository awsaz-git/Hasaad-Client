...
class _HomeDashboardState extends State<HomeDashboard> {
  ...
  Future<void> _loadData({bool showLoading = true}) async {
    ...
    try {
      ...
      final supply = await _service.getCropSupplyMap(profile!.governorateId);
  ...
  Widget build(BuildContext context) {
    ...
    if (_sortBy != 'none') {
      filteredCrops.sort((a, b) {
        switch (_sortBy) {
          case 'pct_high':
            double rA = (_demand[a.id] ?? 0) > 0 ? ((_supply[a.id] ?? 0) / (_demand[a.id] ?? 0)) : 0;
            double rB = (_demand[b.id] ?? 0) > 0 ? ((_supply[b.id] ?? 0) / (_demand[b.id] ?? 0)) : 0;
          ...
          case 'supply_high':
            return (_supply[b.id] ?? 0).compareTo(_supply[a.id] ?? 0);
          case 'supply_low':
            return (_supply[a.id] ?? 0).compareTo(_supply[b.id] ?? 0);
          case 'demand_high':
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Icon(Icons.location_on, size: 14, color: primaryGreen.withOpacity(0.7)),
            ...
            _buildLandUtilizationCard(l10n, darkGreen, primaryGreen),
            const SizedBox(height: 32),
            _buildActionCard(l10n, darkGreen, primaryGreen),
            const SizedBox(height: 32),
            Row(
              children: [
                Text(
                ...
                const Spacer(),
                const Icon(Icons.category_outlined, color: darkGreen, size: 20),
              ],
            ),
            ...
            Row(
              ...
              children: [
                ...
                _buildSortButton(l10n, darkGreen, primaryGreen),
              ],
            ),
  ...
  Widget _buildLandUtilizationCard(AppLocalizations l10n, Color darkGreen, Color primaryGreen) {
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(l10n.translate('total_planted_area'), style: GoogleFonts.cairo(color: Colors.white.withOpacity(0.9), fontSize: 16)),
              const Icon(Icons.info_outline, color: Colors.white70, size: 20),
            ],
          ),
          const SizedBox(height: 8),
          Text(
            '${usedArea.toInt()} / ${totalArea.toInt()} ${l10n.translate('dunums')}',
  ...
  Widget _buildActionCard(AppLocalizations l10n, Color darkGreen, Color primaryGreen) {
    return Column(
      children: [
        GestureDetector(
          onTap: () async {
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(color: primaryGreen.withOpacity(0.4), blurRadius: 15, offset: const Offset(0, 5))
                    ],
                  ),
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: GoogleFonts.cairo(color: primaryGreen, fontWeight: FontWeight.bold, fontSize: 14),
                    ),
                    const SizedBox(width: 4),
                    Icon(Icons.arrow_forward_rounded, size: 18, color: primaryGreen),
                  ],
                ),
              ),
            ),
          ...
          return Opacity(
            ...
            child: Container(
              decoration: BoxDecoration(
                ...
                border: Border.all(color: Colors.red.withOpacity(1 - value), width: 4),
              ),
            ),
          );
        },
  ...
  Widget _buildSortButton(AppLocalizations l10n, Color darkGreen, Color primaryGreen) {
    return PopupMenuButton<String>(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(Icons.tune_rounded, color: darkGreen, size: 18),
            const SizedBox(width: 8),
            Text(
              l10n.translate('filter'),
              style: GoogleFonts.cairo(color: darkGreen, fontWeight: FontWeight.bold, fontSize: 14),
            ),
          ],
        ),
      ),
